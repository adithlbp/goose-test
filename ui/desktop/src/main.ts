...
const getBundledConfig = (): BundledConfig => {
  ...
  return {
    ...
    defaultProviderHost: process.env.GOOSE_DEFAULT_PROVIDER_HOST?.trim() || undefined,
    customProvider: process.env.GOOSE_CUSTOM_PROVIDER?.trim() || undefined,
...
// Corporate bundle: keep the adversary-mode policy in the user's goose config
// dir in sync with the copy shipped in the app resources. The resource is only
// present in corporate builds; the org copy is the source of truth.
const syncBundledAdversaryPolicy = () => {
  if (!app.isPackaged) {
    return;
  }
  try {
    const source = path.join(process.resourcesPath, 'adversary.md');
    ...
    const target = path.join(configDir, 'adversary.md');
    const desired = fsSync.readFileSync(source, 'utf8');
    const current = fsSync.existsSync(target) ? fsSync.readFileSync(target, 'utf8') : null;
    if (current !== desired) {
      fsSync.mkdirSync(configDir, { recursive: true });
      fsSync.writeFileSync(target, desired);
      log.info(`Adversary policy synced to ${target}`);
    }
  } catch (error) {
    log.error('Failed to sync bundled adversary policy:', error);
  }
};
...
let appConfig = {
  ...
  GOOSE_CUSTOM_PROVIDER: customProvider,
    ...
    try {
      gooseServeResult = await startGooseServe({
        ...
        env: {
          ...
          // Corporate bundle: a non-default host makes the provider count as
          // configured server-side without an API key, so onboarding is skipped.
          OPENAI_HOST: defaultProviderHost,
          // Baked as literals by vite `define`: the `...process.env` spread in
          // buildGooseServeEnv cannot carry build-time values to goosed.
          ...
          // Env-first: a baked value pre-answers the telemetry consent prompt
          // and pins the setting (config reads resolve env before config.yaml).
          GOOSE_TELEMETRY_ENABLED: process.env.GOOSE_TELEMETRY_ENABLED,
        },
      ...
      webPreferences: {
        ...
        additionalArguments: [
          JSON.stringify({
            ...
            SECURITY_COMMAND_CLASSIFIER_ENABLED_OVERRIDE:
              process.env.SECURITY_COMMAND_CLASSIFIER_ENABLED_OVERRIDE,
            GOOSE_LOCK_BACKEND: process.env.GOOSE_LOCK_BACKEND,
            GOOSE_LOCK_PROVIDER: process.env.GOOSE_LOCK_PROVIDER,
          }),
        ],
