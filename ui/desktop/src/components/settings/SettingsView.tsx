...
export default function SettingsView({
  ...
  const isBackendLockedByOrg = Boolean(window.appConfig?.get('GOOSE_LOCK_BACKEND'));
  ...
  // Reset active tab if it points to a tab hidden by org policy
  useEffect(() => {
    if (
      (isBackendLockedByOrg && activeTab === 'sharing') ||
      (isProviderLockedByOrg && activeTab === 'auth')
    ) {
      setActiveTab('models');
    }
  }, [isBackendLockedByOrg, isProviderLockedByOrg, activeTab]);
              ...
              <ScrollArea className="flex-1 px-2">
                ...
                >
                  <div className="space-y-8">
                    {CONFIGURATION_ENABLED && !isProviderLockedByOrg && <ConfigSettings />}
