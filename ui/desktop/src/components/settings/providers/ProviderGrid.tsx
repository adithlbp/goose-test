...
const i18n = defineMessages({
  ...
  managedNotice: {
    id: 'providerGrid.managedNotice',
    defaultMessage: 'Providers are managed by your organization and cannot be changed.',
  },
});
...
function ProviderCards({
  ...
  const intl = useIntl();
  const isLockedByOrg = Boolean(window.appConfig?.get('GOOSE_LOCK_PROVIDER'));
  // With the org lock active, only the bundled corporate provider is shown —
  // users must not launch or configure any other provider.
  const pinnedProviderName = useMemo(() => {
    if (!isLockedByOrg) {
      return null;
    }
    ...
    try {
      return (JSON.parse(raw) as { display_name?: string }).display_name ?? null;
    } catch {
      return null;
    }
  }, [isLockedByOrg]);
  ...
  const configureProviderViaModal = useCallback(
    ...
    [openModal, getCurrentModelAndProvider, isLockedByOrg]
  );
  ...
  const providerCards = useMemo(() => {
    ...
    let providersArray = Array.isArray(providers) ? providers : [];
    if (pinnedProviderName) {
      providersArray = providersArray.filter(
        (provider) => provider.metadata.display_name === pinnedProviderName
      );
    }
  ...
  const hasNoMatches =
    query.length > 0 &&
    providerCards.length === (isLockedByOrg ? 0 : 1) &&
  ...
  return (
    <>
      {isLockedByOrg && (
        <div className="mx-auto mb-4 max-w-md px-1 text-center text-xs text-text-secondary italic">
          {intl.formatMessage(i18n.managedNotice)}
        </div>
      )}
